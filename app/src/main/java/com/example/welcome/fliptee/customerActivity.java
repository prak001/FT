    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        spinner.setOnItemSelectedListener(new MaterialSpinner.OnItemSelectedListener<String>() {
            ...
            @Override public void onItemSelected(MaterialSpinner view, int position, long id, String item) {
                ...
                new FetchUrl(getApplicationContext()).execute(item,email);
            }

        });
        SharedPreferences sharedPreferences = getApplicationContext().getSharedPreferences("LOGIN_GOOGLE",0);
        String email = sharedPreferences.getString("email","");
        new FetchUrl(getApplicationContext()).execute("Recomended",email);
...
reflect();
    ...
    class CustomViewHolder////////////////////////////////
        ...
        public void bindData(int position) {
            try {
               // new DownloadImageTask(imageView).execute(arrayList.get(position));
                Glide.with(customerActivity.this).load(arrayList.get(position)).crossFade().diskCacheStrategy(DiskCacheStrategy.ALL).into(imageView);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        ...
        public void onClick(View view) {
            ...
            iv_sticker.setImageBitmap(bm);
            canvas.addView(iv_sticker);
            Toast.makeText(customerActivity.this,"Image Added ",Toast.LENGTH_SHORT).show();
